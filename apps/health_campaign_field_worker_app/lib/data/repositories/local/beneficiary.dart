...
class AddressBeneficaryLocalRepository {
  final LocalSqlDataStore sql;
  final OpLogManager<AddressModel> opLogManager;

  AddressBeneficaryLocalRepository(this.sql, this.opLogManager);
  FutureOr<List<HouseholdModel>> searchHouseHoldbyProximiry(
    ...
    final selectQuery = sql.select(sql.address).join(
      [
        ...
        leftOuterJoin(
          sql.individual,
          sql.individual.clientReferenceId
              .equalsExp(sql.householdMember.individualClientReferenceId),
        ),
        leftOuterJoin(
    ...
    // .where(sql.task.projectBeneficiaryClientReferenceId
    ...
    return results
        .map((e) {
          ...
          final houseHoldMembers = e.readTableOrNull(sql.householdMember);
          final individuals = e.readTableOrNull(sql.individual);
          final projectBeneficiary = e.readTableOrNull(sql.projectBeneficiary);

          final task = e.readTableOrNull(sql.task);
          print(task?.clientReferenceId);
          print("TASK VALUE");
