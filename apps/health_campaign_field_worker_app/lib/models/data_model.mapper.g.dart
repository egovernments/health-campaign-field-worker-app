...
class UserSearchModelMapper extends BaseMapper<UserSearchModel> {
  ...
  UserSearchModel fromMap(Map<String, dynamic> map) => UserSearchModel.ignoreDeleted(userName: Mapper.i.$getOpt(map, 'userName'), uuid: Mapper.i.$getOpt(map, 'uuid'), boundaryCode: Mapper.i.$getOpt(map, 'boundaryCode'));

  @override Function get encoder => (UserSearchModel v) => encode(v);
  dynamic encode(UserSearchModel v) => toMap(v);
  Map<String, dynamic> toMap(UserSearchModel u) => {if (Mapper.i.$enc(u.userName, 'userName') != null) 'userName': Mapper.i.$enc(u.userName, 'userName'), if (Mapper.i.$enc(u.uuid, 'uuid') != null) 'uuid': Mapper.i.$enc(u.uuid, 'uuid'), if (Mapper.i.$enc(u.boundaryCode, 'boundaryCode') != null) 'boundaryCode': Mapper.i.$enc(u.boundaryCode, 'boundaryCode')};

  @override String stringify(UserSearchModel self) => 'UserSearchModel(boundaryCode: ${Mapper.asString(self.boundaryCode)}, isDeleted: ${Mapper.asString(self.isDeleted)}, auditDetails: ${Mapper.asString(self.auditDetails)}, additionalFields: ${Mapper.asString(self.additionalFields)}, userName: ${Mapper.asString(self.userName)}, uuid: ${Mapper.asString(self.uuid)})';
  @override int hash(UserSearchModel self) => Mapper.hash(self.boundaryCode) ^ Mapper.hash(self.isDeleted) ^ Mapper.hash(self.auditDetails) ^ Mapper.hash(self.additionalFields) ^ Mapper.hash(self.userName) ^ Mapper.hash(self.uuid);
  @override bool equals(UserSearchModel self, UserSearchModel other) => Mapper.isEqual(self.boundaryCode, other.boundaryCode) && Mapper.isEqual(self.isDeleted, other.isDeleted) && Mapper.isEqual(self.auditDetails, other.auditDetails) && Mapper.isEqual(self.additionalFields, other.additionalFields) && Mapper.isEqual(self.userName, other.userName) && Mapper.isEqual(self.uuid, other.uuid);
...
abstract class UserSearchModelCopyWith<$R> {
  ...
  $R call({String? userName, List<String>? uuid, String? boundaryCode});
...
class _UserSearchModelCopyWithImpl<$R> extends BaseCopyWith<UserSearchModel, $R> implements UserSearchModelCopyWith<$R> {
  ...
  @override $R call({Object? userName = $none, Object? uuid = $none, Object? boundaryCode = $none}) => $then(UserSearchModel.ignoreDeleted(userName: or(userName, $value.userName), uuid: or(uuid, $value.uuid), boundaryCode: or(boundaryCode, $value.boundaryCode)));
}
