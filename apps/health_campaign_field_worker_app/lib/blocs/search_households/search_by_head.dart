...
class SearchByHeadBloc extends SearchHouseholdsBloc {
  SearchByHeadBloc({
    ...
    on(_handleSearchIndividual);

    on(_handleClearIndividualSearch);
  }

  FutureOr<void> _handleClearIndividualSearch(
    ClearIndividualNameSearchEvent event,
  ...
  FutureOr<void> _handleSearchIndividual(
    SearchIndividualNameEvent event,
    SearchHouseholdsEmitter emit,
  ) async {
    // If search text is empty, emit the existing state and return.
    ...
    // Set loading state and emit the current state.
    super.emit(state.copyWith(
      loading: true,
      searchQuery: event.searchText,
    ));

    // Fetch individuals based on the search text.
    List<IndividualModel> individuals = await individual.search(
    ...
    // Extract individual client reference IDs.
    final individualClientReferenceIds =
        individuals.map((e) => e.clientReferenceId).toList();

    // Fetch project beneficiaries for the individuals.
    List<ProjectBeneficiaryModel> projectBeneficiaries =
        await fetchProjectBeneficiary(individualClientReferenceIds);

    // Fetch tasks related to these project beneficiaries.
    List<TaskModel> tasks = await fetchTaskbyProjectBeneficiary(
      projectBeneficiaries,
    );

    // Fetch side effects based on the tasks.
    List<SideEffectModel> sideEffects = await sideEffectDataRepository.search(
      SideEffectSearchModel(
        taskClientReferenceId: tasks.map((e) => e.clientReferenceId).toList(),
      ),
    );

    // Fetch referrals for the project beneficiaries.
    List<ReferralModel> referrals = await referralDataRepository.search(
      ReferralSearchModel(
    ...
    // Create new containers, preserving the head of household from the event.
    final containers = event.householdMembers.map((wrapper) {
      // Filter tasks, side effects, and referrals for the members.
      final memberIds =
          wrapper.members.map((m) => m.clientReferenceId).toList();

      final individualBeneficiaries = projectBeneficiaries
          .where((beneficiary) =>
              memberIds.contains(beneficiary.beneficiaryClientReferenceId))
          .toList();

      final individualTasks = tasks
          .where((task) => individualBeneficiaries
              .map((b) => b.clientReferenceId)
              .contains(task.projectBeneficiaryClientReferenceId))
          .toList();

      final individualSideEffects = sideEffects
          .where((effect) => individualTasks
              .map((t) => t.clientReferenceId)
              .contains(effect.taskClientReferenceId))
          .toList();

      final individualReferrals = referrals
          .where((referral) => individualBeneficiaries
              .map((b) => b.clientReferenceId)
              .contains(referral.projectBeneficiaryClientReferenceId))
          .toList();

      // Create an updated wrapper for each household member.
      return HouseholdMemberWrapper(
        household: wrapper.household,
        headOfHousehold:
            wrapper.headOfHousehold, // Preserve the head of household.
        members: individuals, // Use the fetched individual members.
        projectBeneficiaries: individualBeneficiaries,
        tasks: individualTasks.isEmpty ? null : individualTasks,
        sideEffects:
            individualSideEffects.isEmpty ? null : individualSideEffects,
        referrals: individualReferrals.isEmpty ? null : individualReferrals,
      );
    }).toList();

    // Emit the new state with the updated household members.
    super.emit(state.copyWith(
      searchQuery: event.searchText,
