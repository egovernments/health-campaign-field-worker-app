...
class ReVerificationBloc
  ...
  // NOTE: non-final on purpose. AuthenticatedPage may swap in a fresh config
  // when MDMS values change (e.g. COUNTDOWN_DURATION_MINUTES update) so the
  // next prompt's countdown / window timers respect the new MDMS values.
  // The bloc captures config.countdownDuration.inSeconds at _startCountdown
  // time, so updating config mid-prompt does NOT shorten the running ticker
  // — only the next prompt sees the change.
  FaceAuthConfig config;

  /// The individual ID of the currently logged-in user.
  /// Face matching during re-verification is scoped to this ID only,
  /// so another enrolled user's embedding on the same device can never
  /// satisfy this user's re-verification check.
  final String currentUserIndividualId;
  ...
  /// Update the in-memory config without recreating the bloc.
  /// Called by AuthenticatedPage when MDMS-derived FaceAuthConfig changes
  /// (e.g. server-side COUNTDOWN_DURATION_MINUTES edit). The next trigger
  /// will use the new values; an in-flight countdown is left as-is.
  void updateConfig(FaceAuthConfig newConfig) {
    config = newConfig;
  }
