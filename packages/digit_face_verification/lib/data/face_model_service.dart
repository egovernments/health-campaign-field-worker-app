...
class FaceDetectionResult {
  ...
  /// Size of the source camera frame (after rotation to upright space).
  /// Used to compute how much of the frame the face fills, which drives
  /// the "Move closer / Move back" distance hint in the capture UI.
  ...
  /// Ratio of face bounding-box width to frame width, in [0, 1].
  /// Roughly: ~0.30 is a comfortable selfie distance, <0.20 is too far,
  /// >0.55 is too close (the face starts clipping the oval).
  double? get faceFillRatio {
    final fw = frameSize?.width;
    if (fw == null || fw <= 0) return null;
    return face.boundingBox.width / fw;
  }
}
...
class FaceModelService {
  ...
  Future<FaceDetectionResult?> processFrame(
    ...
    // After the rotate above, `image` is in the upright frame ML Kit uses,
    // so its dimensions are the correct denominator for face-fill ratio.
