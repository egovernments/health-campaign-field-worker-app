...
class FaceGuideIcon extends StatefulWidget {
  ...
  /// When true, left/right direction is flipped because the back camera
...
class _FaceGuidePainter extends CustomPainter {
  ...
  _FaceGuidePainter({required this.type, required this.animValue, this.isBackCamera = false});

  /// Effective type. Previously this swapped turnLeft ↔ turnRight when the
  /// back camera was active (preview is not mirrored). Per UX feedback the
  /// arrow should now match the instruction text exactly — same direction
  /// for both cameras — so the swap is removed.
  FaceGuideType get _effectiveType => type;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Apply rotation/offset based on effective type (back-camera flip applied)
    canvas.save();
    ...
    if (_effectiveType == FaceGuideType.blink) {
  ...
  bool shouldRepaint(covariant _FaceGuidePainter oldDelegate) =>
      ...
      oldDelegate.isBackCamera != isBackCamera;
}
