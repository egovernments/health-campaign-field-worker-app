...
class ConditionalNavigationExecutor extends ActionExecutor {
  ...
  Future<Map<String, dynamic>> execute(
    ...
    String? screenKey;
    try {
      screenKey = _getScreenKeyFromArgs(context) ?? AutoRouter.of(context).currentPath;
    } catch (_) {
      screenKey = null;
    }
    final currentState = screenKey != null ? FlowCrudStateRegistry().get(screenKey) : null;
    ...
    debugPrint('CONDITIONAL_NAVIGATION: Evaluating ${conditions.length} conditions');
    debugPrint('CONDITIONAL_NAVIGATION: Evaluation context keys: ${evaluationContext.keys}');
    ...
    for (final conditionItem in conditions) {
      ...
      if (conditionItem['default'] == true) {
        ...
        return _executeNavigation(conditionItem['navigateTo'], contextData, evaluationContext);
      }
      ...
      debugPrint('CONDITIONAL_NAVIGATION: Condition "$condition" evaluated to $isConditionTrue');

      if (isConditionTrue) {
        // Check if we also need to verify additionalField is missing
        final checkAdditionalFieldMissing = conditionItem['checkAdditionalFieldMissing'] as String?;
        if (checkAdditionalFieldMissing != null) {
          final isFieldMissing = _isAdditionalFieldMissing(contextData, checkAdditionalFieldMissing);
          debugPrint('CONDITIONAL_NAVIGATION: additionalField "$checkAdditionalFieldMissing" missing = $isFieldMissing');
          if (!isFieldMissing) {
  ...
  Map<String, dynamic> _buildEvaluationContext({
    ...
    if (contextData.containsKey('existingModels')) {
      ...
      if (existingModels is List && existingModels.isNotEmpty) {
        debugPrint('CONDITIONAL_NAVIGATION: Found ${existingModels.length} existingModels');
        for (int i = 0; i < existingModels.length; i++) {
          ...
          if (model is EntityModel) {
            ...
            debugPrint('CONDITIONAL_NAVIGATION: Added $i.$typeName from existingModels');
            debugPrint('CONDITIONAL_NAVIGATION: $typeName.symptom = ${modelMap['symptom']}');
            debugPrint('CONDITIONAL_NAVIGATION: $typeName.additionalFields = ${modelMap['additionalFields']}');
          }
        }
      }
    }
    ...
    if (contextData.containsKey('item')) {
      ...
      if (item is Map) {
        ...
        if (!evalContext.containsKey(key0)) {
          ...
          evalContext[typeName] = modelMap;
          debugPrint('CONDITIONAL_NAVIGATION: Added $key0 from item (EntityModel)');
        }
      }
    }
        ...
        for (int i = 0; i < wrappers.length; i++) {
          ...
          if (wrapper is Map) {
            ...
            if (!evalContext.containsKey(keyI)) {
              ...
              debugPrint('CONDITIONAL_NAVIGATION: Added $keyI from wrappers (EntityModel)');
            }
          }
        }
      }
        ...
        if (item is Map) {
          item.forEach((entityType, entityData) {
            ...
            if (!evalContext.containsKey(keyI)) {
              ...
              debugPrint('CONDITIONAL_NAVIGATION: Added $keyI from stateWrapper');
            }
          });
        } else if (item is EntityModel) {
          final typeName = getEntityTypeName(item);
          ...
          if (!evalContext.containsKey(keyI)) {
            ...
            debugPrint('CONDITIONAL_NAVIGATION: Added $keyI from stateWrapper (EntityModel)');
          }
        }
      }
    }

    debugPrint('CONDITIONAL_NAVIGATION: Final evalContext keys: ${evalContext.keys}');
    return evalContext;
  }
  ...
  dynamic _entityToMap(dynamic entity) {
    if (entity is EntityModel) {
      ...
      debugPrint('CONDITIONAL_NAVIGATION: Converted EntityModel to map with keys: ${map.keys}');
      return map;
    }
    if (entity is Map) {
  ...
  bool _evaluateSingleCondition(String condition, Map<String, dynamic> context) {
    // Handle == comparison
    if (condition.contains('==')) {
      final parts = condition.split('==').map((p) => p.trim()).toList();
      if (parts.length == 2) {
        ...
        debugPrint('CONDITIONAL_NAVIGATION: Comparing path "$leftPath" (value: $actualValue) == $expectedValue');
  ...
  dynamic _resolvePathValue(String path, Map<String, dynamic> context) {
    ...
    debugPrint('CONDITIONAL_NAVIGATION: Resolving path "$path" with ${parts.length} parts');
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      if (current is Map) {
        ...
        if (current.containsKey(part)) {
          ...
          debugPrint('CONDITIONAL_NAVIGATION: Found key "$part", new value type: ${current.runtimeType}');
          continue;
        }
        ...
        for (int j = i + 1; j <= parts.length; j++) {
          ...
          if (current.containsKey(compoundKey)) {
            ...
            debugPrint('CONDITIONAL_NAVIGATION: Found compound key "$compoundKey", new value type: ${current.runtimeType}');
        ...
        if (part == 'fields' && current.containsKey('fields')) {
          ...
          debugPrint('CONDITIONAL_NAVIGATION: Accessed fields list, length: ${current is List ? current.length : 'not a list'}');
          continue;
        }
      ...
      if (current is List) {
        ...
        for (final item in current) {
          if (item is Map && item['key'] == part) {
            found = item['value'];
            debugPrint('CONDITIONAL_NAVIGATION: Found AdditionalField with key "$part", value: $found');
            break;
          }
        }
        ...
        debugPrint('CONDITIONAL_NAVIGATION: Key "$part" not found in additionalFields list');
        return null;
      }
  ...
  bool _isAdditionalFieldMissing(Map<String, dynamic> contextData, String fieldKey) {
        ...
        for (final model in existingModels) {
          if (model is EntityModel) {
            ...
            if (additionalFields != null && additionalFields is Map) {
              ...
              if (fields is List) {
                for (final field in fields) {
                  if (field is Map && field['key'] == fieldKey && field['value'] != null) {
                    debugPrint('CONDITIONAL_NAVIGATION: Found additionalField "$fieldKey" with value: ${field['value']}');
    ...
    debugPrint('CONDITIONAL_NAVIGATION: additionalField "$fieldKey" is missing or null');
  ...
  Map<String, dynamic> _executeNavigation(
    ...
    debugPrint('CONDITIONAL_NAVIGATION: Navigating to $targetName (type: $targetType, flow: $flowName)');
    ...
    if (targetType?.toUpperCase() == 'FORM' && flowName != null) {
      ...
      FlowCrudStateRegistry().updateNavigationParams(flowScreenKey, navParamsWithPage);
      FlowCrudStateRegistry().updateNavigationParams(pageScreenKey, navParamsWithPage);
      FlowCrudStateRegistry().updateNavigationParams(flowName, navParamsWithPage);
      FlowCrudStateRegistry().updateNavigationParams(targetName, navParamsWithPage);
      ...
      debugPrint('CONDITIONAL_NAVIGATION: Using FORM navigation with flow=$flowName, startPage=$targetName');
      debugPrint('CONDITIONAL_NAVIGATION: Navigation data: $resolvedDataList');
      debugPrint('CONDITIONAL_NAVIGATION: Full navigation properties: $navigationProperties');
      debugPrint('CONDITIONAL_NAVIGATION: navParamsWithPage stored: $navParamsWithPage');
