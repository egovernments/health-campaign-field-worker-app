...
import 'search_entity_repository.dart';
...
class HydrationHelper {
  static Future<List<Map<String, dynamic>>> hydrateRawRows(
    ...
    for (final entry in modelNestedMapping.entries) {
      ...
      final localValues = enrichedRows.map((row) => row[localKeySnake]).where((v) => v != null).toSet();
      ...
      for (final row in enrichedRows) {
        ...
        final relatedRows = targetResults.where((targetRow) {
          return targetRow[repo._queryBuilder.camelToSnake(field.foreignKey)] == localValue;
        }).toList();
