  ...
  static Future<List<Map<String, dynamic>>> queryRawTable({
    ...
    for (final filter in filters.where((f) => f.root == table)) {
      if (filter.operator == 'within') {
        ...
        final deltaLon = radiusInKm / (earthRadius * (centerLat * degToRad).cos());
        ...
        final boundingBox = latExpr.isBetweenValues(minLat, maxLat) & lonExpr.isBetweenValues(minLon, maxLon);
    ...
    if (isPrimaryTable && onCountFetched != null) {
      final whereClause = buildWhereClauseRaw(filters.where((f) => f.root == table).toList());
      final whereArgs = buildWhereArgs(filters.where((f) => f.root == table).toList());
      int finalCount;
      if (centerLat != null && centerLon != null && radiusInKm != null) {
        ...
        double haversine(double lat1, double lon1, double lat2, double lon2) {
          ...
          final a = (dLat / 2).sin() * (dLat / 2).sin() +
              (lat1 * degToRad).cos() * (lat2 * degToRad).cos() * (dLon / 2).sin() * (dLon / 2).sin();
