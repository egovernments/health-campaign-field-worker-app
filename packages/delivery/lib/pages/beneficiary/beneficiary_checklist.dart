...
import 'package:delivery/widgets/back_navigation_help_header.dart';
    ...
    context.read<ServiceBloc>().add(
          ServiceCreateEvent(
            serviceModel: ServiceModel(
              ...
              rowVersion: 1,
              accountId: DeliverySingleton().projectId,
              additionalDetails: {
                "boundaryCode": DeliverySingleton().boundary?.code
              },
              additionalFields: ServiceAdditionalFields(version: 1, fields: [
                AdditionalField(
                    'relatedClientReferenceId', widget.beneficiaryClientRefId),
                AdditionalField(
                    'localityCode', DeliverySingleton().boundary!.code),
                if (decidedFlow != null)
    ...
    switch (status) {
      ...
      case Status.toAdminister:
        if (selectedServiceDefinition!.code!
          ...
          ctx.router.navigate(BeneficiaryDetailsDeliveryRoute());
        }
        break;
      default:
