...
typedef HouseholdOverviewEmitter = Emitter<HouseholdOverviewDeliveryState>;

class HouseholdOverviewDeliveryBloc
    extends Bloc<HouseholdOverviewEvent, HouseholdOverviewDeliveryState> {
  ...
  final IndividualGlobalDeliverySearchRepository
  ...
  FutureOr<void> _selectedIndividual(
    ...
    Emitter<HouseholdOverviewDeliveryState> emit,
  ) async {
  ...
  FutureOr<void> _handleReloadMember(
    ...
    if (event.searchByName == null &&
          ...
          ? await householdMemberRepository.search(
              HouseholdMemberSearchModel(
                householdClientReferenceId:
                    state.householdMemberDeliveryWrapper.household != null
                        ? [
                            state.householdMemberDeliveryWrapper.household!
                                .clientReferenceId
                          ]
                        : [],
                limit: event.limit,
      ...
      final householdId =
          state.householdMemberDeliveryWrapper.household?.clientReferenceId;
      ...
      if (head == null) {
        ...
        emit(state.copyWith(
          ...
          householdMemberDeliveryWrapper:
              state.householdMemberDeliveryWrapper.copyWith(
            members:
                (event.projectBeneficiaryType == BeneficiaryType.individual)
                    ? [
                        ...state.householdMemberDeliveryWrapper.members ?? [],
                        ...beneficiaryIndividuals,
                      ]
                    : [
                        ...state.householdMemberDeliveryWrapper.members ?? [],
            ...
            projectBeneficiaries: [
              ...state.householdMemberDeliveryWrapper.projectBeneficiaries ??
                  [],
              ...projectBeneficiaries,
            ],
            tasks: [
            ...
            sideEffects: [
              ...?state.householdMemberDeliveryWrapper.sideEffects,
              ...sideEffects,
            ],
            referrals: [
              ...?state.householdMemberDeliveryWrapper.referrals,
              ...referrals,
            ],
          ),
        ));
      ...
      final totalResult = await individualGlobalSearchRepository
          .individualGlobalSearch(GlobalSearchParameters(
              householdType: DeliverySingleton().householdType,
              householdClientReferenceId: state
                  .householdMemberDeliveryWrapper.household!.clientReferenceId,
      ...
      if (result.isEmpty) {
        emit(state.copyWith(
          loading: false,
          householdMemberDeliveryWrapper: HouseholdMemberDeliveryWrapper(
            household: state.householdMemberDeliveryWrapper.household,
            headOfHousehold:
                state.householdMemberDeliveryWrapper.headOfHousehold,
            members: [],
            tasks: null,
      ...
      if (event.selectedFilter!.contains(Status.registered.name) ||
        ...
        List<ProjectBeneficiaryModel> projectBeneficiariesList =
            await projectBeneficiaryRepository.search(
                ProjectBeneficiarySearchModel(
                    projectId: [DeliverySingleton().projectId.toString()],
...
@freezed
class HouseholdOverviewDeliveryState with _$HouseholdOverviewDeliveryState {
  const factory HouseholdOverviewDeliveryState({
    @Default(false) bool loading,
    required HouseholdMemberDeliveryWrapper householdMemberDeliveryWrapper,
    IndividualModel? selectedIndividual,
    int? offset,
    int? limit,
  }) = _HouseholdOverviewDeliveryState;
}
