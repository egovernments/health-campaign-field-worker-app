...
class IndividualDetailsPageState extends LocalizedState<IndividualDetailsPage> {
  ...
  void initState() {
    if (RegistrationDeliverySingleton().idTypeOptions!.length == 1) {
      RegistrationDeliverySingleton()
          .idTypeOptions!
          .add(IdentifierTypes.uniqueBeneficiaryID.name.toString());
                                ...
                                builder: (field) => LabeledField(
                                  ...
                                  child: RadioList(
                                    ...
                                    errorMessage: form
                                        ...
                                        ? localizations.translate(
                                            i18.individualDetails
                                                .doesChildHaveVaccinationConditionError,
                                          )
                                        : null,
                                    radioDigitButtons: [
  ...
  IndividualModel _getIndividualModel(
    ...
    int noOfMonthsPregnant =
        form.control(_noOfPregnantMonthsKey).value as int? ?? 0;
    int noOfTimesVisitedHF =
        form.control(_noOfTimesVisitedHFKey).value as int? ?? 0;
  ...
  FormGroup buildForm(BeneficiaryRegistrationState state) {
    ...
    return fb.group(<String, Object>{
      ...
      _ttVaccinesTakenKey: FormControl<int>(
        value: int.tryParse((individual?.additionalFields?.fields
                        .where((field) => field.key == _ttVaccinesTakenKey)
                        .firstOrNull
                        ?.value ??
                    0)
                .toString()) ??
      ...
      _noOfPregnantMonthsKey: FormControl<int>(
        value: int.tryParse((individual?.additionalFields?.fields
                        .where((field) => field.key == _noOfPregnantMonthsKey)
                        .firstOrNull
                        ?.value ??
                    0)
                .toString()) ??
      ...
      _noOfTimesVisitedHFKey: FormControl<int>(
        value: int.tryParse((individual?.additionalFields?.fields
                        .where((field) => field.key == _noOfTimesVisitedHFKey)
                        .firstOrNull
                        ?.value ??
                    0)
                .toString()) ??
      ...
      _noOfChildrenLessThan5Key: FormControl<int>(
        value: int.tryParse((individual?.additionalFields?.fields
                        .where(
                            (field) => field.key == _noOfChildrenLessThan5Key)
                        .firstOrNull
                        ?.value ??
                    0)
                .toString()) ??
