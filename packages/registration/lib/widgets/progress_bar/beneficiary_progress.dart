...
import 'package:registration/data/repositories/local/base/project_beneficiary.dart';
import 'package:registration/widgets/progress_indicator/progress_indicator.dart';
...
class BeneficiaryProgressBarState extends State<BeneficiaryProgressBar> {
  ...
  void didChangeDependencies() {
    ...
    repository.listenToChanges(
      query: ProjectBeneficiarySearchModel(
        projectId: [RegistrationSingleton().projectId.toString()],
      ),
      listener: (data) {
        if (mounted) {
          setState(() {
  ...
  Widget build(BuildContext context) {
    final selectedProject = RegistrationSingleton().selectedProject!;
